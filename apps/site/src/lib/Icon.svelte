<script lang="ts" module>
  // Lucide-style 24x24 stroke icons, inlined to keep the site dependency-free.
  ...
  // Maps free-form icon names (e.g. "Rocket", "book-open") onto our set.
  export function normalizeIcon(name: string | undefined): string | undefined {
    if (!name) return undefined
    if (name in paths) return name
    const key = name.toLowerCase().replace(/[-_\s]/g, '')
    const found = Object.keys(paths).find((k) => k.toLowerCase() === key)
...
<script lang="ts">
  let { name, size = 16, class: klass = '' }: { name: string; size?: number; class?: string } = $props()
</script>

...
- **Services are JSON APIs only.** No Inertia, no server-rendered pages, no
  ...
  Every UI is a separate Svelte 5 + Vite app in `apps/` (see `CLAUDE.md`),
  which talks to the service over HTTP.
- Return data with `ctx.Response().Json(...)`; never `View().Make(...)`.
- Delete scaffold leftovers that serve pages (welcome view, `resources/`,
  `public/` assets) rather than keeping them around.
- Local dependencies run with Podman (`infra/dev/`), not Docker. Upstream's
  `docker-compose.yml`/`Dockerfile` are replaced by files under `infra/`.
